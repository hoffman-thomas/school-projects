	...
	*/
    ...
    //.
		...
		while(unSettledNodes.isEmpty() == false){
			...
			Vertex min = getMinimum(unSettledNodes);

			settledNodes.add(min);
			unSettledNodes.remove(min);
			findMinimalDistances(min);
    ...
    private void findMinimalDistances(Vertex node) {
		...
		for(Vertex neighbor: adjacentNodes){
			if(getShortestDistance(neighbor) > getShortestDistance(node)+getDistance(node,neighbor)){
